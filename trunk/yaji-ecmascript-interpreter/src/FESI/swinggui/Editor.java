...
public class Editor extends JPanel {
    ...
    private static final long serialVersionUID = -4093881123234545031L;
    ...
    /**
     * Check if the current content must be saved - return true if the operation
     * must be aborted
     */
    boolean checkDirty() {
        if (dirty) {
            int result = JOptionPane.showConfirmDialog(this, "Save changes in "
                    + getWindowBaseTitle() + " ?", "FESI Editor",
    ...
    protected JMenuBar createMenubar() {
        ...
        }, KeyStroke.getKeyStroke(KeyEvent.VK_F4, 0), WHEN_IN_FOCUSED_WINDOW);
    ...
    public String getWindowBaseTitle() {
        ...
        String title = (currentFile == null ? defaultTitle : currentFile
                .getPath());
        return title;
    }
    ...
    public String getWindowTitle() {
        String title = getWindowBaseTitle() + (dirty ? " *" : "");
        return title;
    }

    // Refresh the title, using a standard title if no file name known and *
    // indicator
    ...
    /**
     * Process the Open command, check if the buffer must be saved, then ask for
     * a file and load its content
     */
    void commandOpen() {
        if (checkDirty())
            return;
    ...
    /**
     * Ask a new file name in which to save the buffer content then save the
     * buffer.
     */
    void commandSaveAs() {
        Frame frame = getFrame();
    ...
    /**
     * Save the buffer if required, then clear the content to start a new
     * document
     */
    void commandNew() {
        if (checkDirty())
    ...
    /**
     * Thread to load a file into the text storage model (From the Notepad
     * example from Sun - not really usefull here)
     */
    class FileLoader extends Thread {
        ...
        public void run() {
            try {
                 ...
                 * progress.setMinimum(0); progress.setMaximum((int)
                 * f.length()); status.add(progress); status.revalidate();
                 */
