...
/**
 * <p>
 * The Code attribute is defined to describe the implementation for each Method
 * Implementation in a class. In particular it contains the byte codes and
...
final class Code extends Attribute {
     ...
     * 
     * @param locals
     *            number of words used to describe local vars
     * @param maxstack
     *            max number of stack words used.
     * 
     */
    ...
    /**
     * <p>
     * adds per Code attribute, can be used for SourceFile, LocalVariable, and
     * LineNumberTable attributes etc.
     * </p>
     * 
     * @param attr
     *            Attribte to be added.
     */

    void addAttribute(Attribute attr) {
        if (attributes == null)
            attributes = new Vector<Attribute>(1);
    ...
    /**
     * <p>
     * Adds an entry to the Exception handler table for this Code attribute. An
     * entry describes the start and stop pc offset within the Code fragment for
     * which an exception handler is provided, the start pc of the handler code
     * within the fragment itself, and the class of the exception type for this
     * handler.
     * </p>
     * 
     * @param start
     *            start pc offset for this exception handler range
     * @param stop
     *            stop pc offset for this exception handler range
     * @param handler
     *            handler pc start offset for this exception
     * @param ct
     *            CONSTANT_CLASS describing the exception class handled
     */

    void addExceptionTableEntry(short start, short stop, short handler,
            ClassConstant ct) {
        exceptions
                .addElement(new ExceptionTableEntry(start, stop, handler, ct));
    ...
    void addOpShort(byte opCode, short op) {
        addOp2(opCode, (byte) ((op >>> 8) & 0xff), (byte) (op & 0xff));
    }
    ...
    void addOpInt(byte opCode, int op) {
        addOp4(opCode, (byte) ((op >>> 24) & 0xff),
                (byte) ((op >>> 16) & 0xff), (byte) ((op >>> 8) & 0xff),
                (byte) (op & 0xff));
    }
     ...
     * 
     * @param n
     *            the number of local words to increment by.
     */

    void incrLocals(short n) {
        maxLocals += n;
    }
     ...
     * 
     * @param n
     *            the number of words to increment the max stack count by
     */

    void incrMaxStack(short n) {
        maxStack += n;
    }
}
...
final class ExceptionTableEntry {
    ...
    ExceptionTableEntry(short start, short stop, short handler,
            ClassConstant eType) {
        super();
