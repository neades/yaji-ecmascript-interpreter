...
/**
 * <p>
 * This class is used by the EventAdaptorGenerator to author the implementation
 * of the EventAdaptor classes that it is responsible for generating and
 * loading.
 * </p>
 * 
 * <p>
 * This class wraps all the ClassFile generic support classes and provides the
 * adaptor specific implemenetation.
 * </p>
 * 
...
public final class EventAdaptorClassFile {
    ...
    /**
     * <p>
     * This statis function creates the class file implementation and writes it
     * to the stream for loading ...
     * </p>
     * 
     * @param className
     *            the name of the adaptor class to synthesize.
     * @param os
     *            the stream to write the class file into
     * @exception IOException
     *                If any IO error occured during class loading
    ...
    public EventAdaptorClassFile(String className, OutputStream os)
        ...
        listenerName = EventAdaptorGenerator
                .getBaseNameFromAdaptorName(className);
    ...
    private void generateClassSundries() {
        ...
        classFile.addFieldDesc(new FieldDesc("methods",
                "[Ljava/lang/reflect/Method;",
                (short) (FieldDesc.ACC_STATIC | FieldDesc.ACC_PRIVATE),
                classFile, (Attribute[]) null));

        methodsField = classFile.addFieldConstant(adaptorName, "methods",
                "[Ljava/lang/reflect/Method;");
        ...
        getMethodsMethod = classFile.addMethodConstant("java/lang/Class",
        ...
        forNameMethod = classFile.addMethodConstant("java/lang/Class",
                "forName", "(Ljava/lang/String;)Ljava/lang/Class;");
        /*
         * private static java.lang.Class clazz;
         */

        classFile.addFieldDesc(new FieldDesc("clazz", "Ljava/lang/Class;",
                (short) (FieldDesc.ACC_STATIC | FieldDesc.ACC_PRIVATE),
                classFile, (Attribute[]) null));

        clazzField = classFile.addFieldConstant(adaptorName, "clazz",
                "Ljava/lang/Class;");
        ...
        fireMethod = classFile.addMethodConstant(adaptorName, "fire",
                "(Ljava/util/EventObject;Ljava/lang/reflect/Method;)V");

        crackedFireMethod = classFile.addMethodConstant(adaptorName, "fire",
    ...
    private void generateInitializer() {
        ...
        c.addOpShort(Code.OP_INVOKE_STATIC, forNameMethod
                .getConstantPoolIndex());

        c.addOp(Code.OP_DUP);

        c.addOpShort(Code.OP_PUTSTATIC, clazzField.getConstantPoolIndex());

        // methods = clazz.getMethods();

        c.addOpShort(Code.OP_INVOKE_VIRTUAL, getMethodsMethod
                .getConstantPoolIndex());

        c.addOpShort(Code.OP_PUTSTATIC, methodsField.getConstantPoolIndex());

        c.addOp(Code.OP_RETURN);

        classFile.addMethodDesc(new MethodDesc("<clinit>", "()V",
    ...
    private void generateConstructor() {
        ...
        mc = classFile.addMethodConstant(ClassFile
                .fullyQualifiedForm(superClassName), "<init>", "()V");
    ...
    /**
     * Author the Listener Method Stubs for the EventListener interface this
     * class is adapting to the EventListener interface.
     */
    ...
    private void generateListenerMethods() {
        for (int i = 0; i < listenerMethods.length; i++) {
            ...
            /*
             * we can only generate code for EventListener methods of the form:
             * 
             * void <method_name> ( <EventObject Subtype> ) or: void
             * <method_name> ( {<arbitrary arg list>} )
             * 
             * if we dont match these patterns we drop the method on the floor.
             */

            if (!Void.TYPE.equals(listenerMethods[i].getReturnType())) {
                System.err
                        .println("Detected unexpected method signature: "
                                + listenerMethods[i] + " in interface: "
                                + listenerName);
            } else {
                Class<?>[] lmParams = listenerMethods[i].getParameterTypes();
                Class<?>[] lmExceptions = listenerMethods[i]
                        .getExceptionTypes();

                if (lmParams != null
                        && lmParams.length == 1
                        && java.util.EventObject.class
                                .isAssignableFrom(lmParams[0])) {
                    generateSimpleListenerMethodStub(listenerMethods[i],
                            lmParams[0], lmExceptions, i);
                } else {
                    generateCrackedListenerMethodStub(listenerMethods[i],
                            lmParams, lmExceptions, i);
                }
            }
        }
    }
    ...
    private void generateSimpleListenerMethodStub(Method listenerMethod,
            Class<?> listenerParam, Class<?>[] listenerExceptions,
            int listenerMethodTableIndex) {
        Code c = new Code(classFile, (short) 2, (short) 4);
        Attribute[] ary;

        /*
         * public void <EventListenerMethod>(<EventObject> e) {
         * EncapsulatedEvent t = new EncapsulatedEvent(e); Method m =
         * findListenerMethod();
         * 
         * fire(t, m); }
         */
        ...
        c.addOpShort(Code.OP_GETSTATIC, methodsField.getConstantPoolIndex());

        if (listenerMethodTableIndex <= 255) {
            c.addOp1(Code.OP_BIPUSH, (byte) listenerMethodTableIndex);
        } else {
            short i = classFile.addIntegerConstant(listenerMethodTableIndex)
                    .getConstantPoolIndex();
        ...
        c.addOpShort(Code.OP_INVOKE_VIRTUAL, fireMethod.getConstantPoolIndex()); // call
                                                                                 // fire();
        ...
        classFile
                .addMethodDesc(new MethodDesc(
                        listenerMethod.getName(),
                        "(" + ClassFile.fieldType(listenerParam.getName())
                                + ")V",
    ...
    private void generateCrackedListenerMethodStub(Method listenerMethod,
            Class<?>[] listenerParams, Class<?>[] listenerExceptions,
            int listenerMethodTableIndex) {
        Code c = new Code(classFile, (short) (listenerParams.length * 2 + 1),
                (short) 9);
        Attribute[] ary;
        String methodPDesc = "";
        ...
        if (listenerParams.length <= 255) {
            ...
            short i = classFile.addIntegerConstant(listenerParams.length)
                    .getConstantPoolIndex();
        ...
        c.addOpShort(Code.OP_ANEWARRAY, classFile.addClassConstant(
                "java.lang.Object").getConstantPoolIndex());
        ...
        /*
         * for each formal parameter, generate code to load the actual param
         * from this methods local vars, then if it is a primitive type, then
         * construct a container object and initialize it to the primitives
         * value.
         * 
        ...
        for (int i = 0; i < listenerParams.length; i++, lvarIdx += (wasDoubleWord ? 2
                : 1)) {
            ...
            if (lvarIdx <= 255) { // the array index
                ...
                short ic = classFile.addIntegerConstant(i)
                        .getConstantPoolIndex();
                if (ic < 255)
            ...
            /*
             * get the param value onto TOS as a side effect gather method
             * descriptor string.
             */
        ...
        c.addOpShort(Code.OP_INVOKE_VIRTUAL, crackedFireMethod
                .getConstantPoolIndex()); // call fire();
        ...
        classFile
                .addMethodDesc(new MethodDesc(listenerMethod.getName(), "("
                        + methodPDesc + ")V", (short) (listenerMethod
                        .getModifiers() & ~MethodDesc.ACC_ABSTRACT), classFile,
                        ary));
    }

    /*
     * This method is used to generate code for cracked event listener stubs.
     * Its job is to generate code to load the appropriate parameter data type
     * onto the stack, create a wrapper object if needed and leave the
     * appropriate value on TOS for storing into the objects array.
     */
    ...
    private String processParam(Code c, Class<?> pClass, int pIdx) {
        ...
        if (isPrimitive) { // builtin datatype
            if (pType.equals(java.lang.Long.TYPE)) {
                ...
                if (!isArrayRef) {
                    ...
                    mc = classFile.addMethodConstant("java/lang/Float",
                            "<init>", "(F)V");
                ...
                if (!isArrayRef) {
                    ...
                    mc = classFile.addMethodConstant("java/lang/Double",
                            "<init>", "(D)V");
                ...
                if (pType.equals(java.lang.Boolean.TYPE)) {
                    ...
                    if (!isArrayRef) {
                        ...
                        mc = classFile.addMethodConstant("java/lang/Boolean",
                                "<init>", "(Z)V");
                    ...
                    if (!isArrayRef) {
                        ...
                        mc = classFile.addMethodConstant("java/lang/Character",
                                "<init>", "(C)V");
                    ...
                    if (!isArrayRef) {
                        ...
                        mc = classFile.addMethodConstant("java/lang/Short",
                                "<init>", "(S)V");
                    ...
                    if (!isArrayRef) {
                        ...
                        mc = classFile.addMethodConstant("java/lang/Integer",
                                "<init>", "(I)V");
                    }
                }
            }
        ...
        if (isPrimitive && !isArrayRef) { // additional processing for
                                          // primitives
