...
/**
 * Currently a reference is not a FESI value, as it can never be returned as a
 * valid value by an EcmaScript function (8.7). It would be easy to make
 * ESReference a subclass of ESValue or to create a common superclass (ESData)
 * for both ESReference and ESValue if so desired in the future (in fact this
 * was the case in the first version of the system).
 * <P>
 * References are less used than in the standard, as they are built only if
 * required for assignement.
 */
public class ESReference {
    ...
    /**
     * Return the referenced value unless it is global and not defined, in which
     * case an exception is raised (see 8.7.3). By the definition of
     * getProperty, and undefined value is returned if the base object is
     * defined but the property does not exist (see 8.6.2.1).
     * 
    ...
    /**
     * Update the referenced value, creating it if needed. If the base is is
     * null use the global object, otherwise use the base object. See 8.7.4.
     * 
     * @param g
     *            The global object to use if thre base is null
     * @param v
     *            The value to put
     * @exception EcmaScriptException
     *                May be thrown by putProperty
     */
    ...
    /**
     * Return the name of the reference, using a dot notation (could use the
     * bracket notation, or check which one is most appropriate). Note that the
     * base object is returned between braces, we do not know its name at this
     * stage. This is used for debugging purpose, there is no way to get the
     * string of a reference in EcmaScript.
     * 
