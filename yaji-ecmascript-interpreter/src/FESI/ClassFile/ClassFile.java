...
/**
 * <p>
 * The ClassFile class is designed to provide lightweight, minimal support for
 * the runtime construction of Java VM ClassFile's, or Class implementations.
 * </p>
 * <p>
 * The ClassFile provides API's to construct an in-core description of a Java
 * class implementation, and subsequently write that description to a stream
 * which may then be either loaded into the VM via a ClassLoader or written to
 * some persistent store.
 * </p>
 * <p>
 * It should be noted that the ClassFile provide little or no validation of the
 * Class it describes during the construction of that description, and therefore
 * users of this class and package should be familiar with the contents of the
 * Java VM Specification published by Addison-Wesley.
 * </p>
 */
...
final class ClassFile {
    ...
    private Vector<ConstantPoolEntry> constantPool = new Vector<ConstantPoolEntry>(
            1);
    ...
    private void writeConstantPool(DataOutputStream dos) throws IOException {
        ...
        if (debug())
            System.err.println("write constant pool: " + constantPool.size());
    ...
    private void writeInterfaces(DataOutputStream dos) throws IOException {
        if (interfaces != null) {
            if (debug())
                System.err.println("write interfaces: " + interfaces.size());
            dos.writeShort(interfaces.size());

            for (int i = 0; i < interfaces.size(); i++) {
                dos.writeShort(((ConstantPoolEntry) interfaces.elementAt(i))
    ...
    private void writeFields(DataOutputStream dos) throws IOException {
        if (fields != null) {
            if (debug())
                System.err.println("write fields: " + fields.size());
    ...
    private void writeMethods(DataOutputStream dos) throws IOException {
        if (methods != null) {
            if (debug())
                System.err.println("write methods: " + methods.size());
    ...
    private void writeAttributes(DataOutputStream dos) throws IOException {
        if (attributes != null) {
            if (debug())
                System.err.println("write attributes: " + attributes.size());
    ...
    public synchronized void write(OutputStream os) throws IOException {
        ...
        try {
            ...
            if (debug())
                System.err.println("access: " + accessFlags);
     ...
     * 
     * @param tag
     *            The tag value of the constant pool entries to match on.
     * @param value
     ...
     * 
     * @param newf
     *            the new flag values. [NOT VALIDATED]
     */
    ...
    /**
     * @param newMethod
     *            the method desc to add to the class file.
     */

    public synchronized void addMethodDesc(MethodDesc newMethod) {
        if (methods == null)
            methods = new Vector<MethodDesc>(1);
    ...
    /**
     * @param newField
     *            the field desc to add to the class file.
     */

    public synchronized void addFieldDesc(FieldDesc newField) {
        if (fields == null)
            fields = new Vector<FieldDesc>(1);
    ...
    /**
     * @param sConstant
     *            add a CONSTANT_STRING to the ClassFile.
     * 
     * @param sConstant
    ...
    public StringConstant addStringConstant(String sConstant) {
        UTF8Constant c = (UTF8Constant) match(ConstantPoolEntry.CONSTANT_UTF8,
                sConstant);
    ...
    public ClassConstant addClassConstant(String classConstant) {
        ClassConstant c = (ClassConstant) match(
                ConstantPoolEntry.CONSTANT_CLASS, classConstant);
     ...
     *            the method name
     * @param tName
     *            the fully qualified type descriptor for the method
     * 
     * @return the new created CONSTANT_METHOD
     */

    public MethodConstant addMethodConstant(String cName, String mName,
            String tName) {
     ...
     * @param tName
     *            the fully qualified type descriptor of the field
     * 
     * @return the new created CONSTANT_FIELD
     */

    public FieldConstant addFieldConstant(String cName, String fName,
            String tName) {
    ...
    public void addInterface(String iName) {
        if (interfaces == null)
            interfaces = new Vector<Object>(1);
    ...
    /**
     * <p>
     * convenience routine to construct type descriptors from fully qualified
     * names, e.g: java.lang.Object => Ljava/lang/Object;
     * </p>
     * 
     * @param str
     *            name of a java "type"
