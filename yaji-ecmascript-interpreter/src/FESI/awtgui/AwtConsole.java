...
public class AwtConsole extends Frame implements Console {
  ...
  /**
   * Create a new console interface - this is limited by the AWT usage
   * of TextEdit.
   * @param itrpParam The interpreter which will receive the exit
   * command whel EOF is received.
   * @param title The window title string
   * @rows Starting number of rows
   * @columns Starring number of columnes
   */
  public AwtConsole(FESI.gui.InterpreterCommands itrpParam, String title, int rows, int columns) {
    super(title);
  ...
  public void displayHelpWindow() {
   ...
   static boolean needsAppend()
   {
    ...
    /**
       ...
       everything is passed along. If backspace (ch = 8) is received then
       the last character in the buffer is removed.
     */
    ...
    public class TextAreaInputStream extends PipedInputStream
      ...
      synchronized public void keyTyped(KeyEvent keyEvent) {
        ...
        try {
          ...
          // System.err.println("Keychar: " + int( ch ) + ": " + keyEvent.toString());
    
          if (ch == 8) { // Backspace
            if (nbrOfKeyTyped > 0) {
              //String text = theTextArea.getText();
              theTextArea.append(""); // Used for placing the cursor at the end.
              //int caret = theTextArea.getCaretPosition();
              //theTextArea.setText(text.substring(0, text.length() - 1));
              //theTextArea.append(""); // Used for placing the cursor at the end.
              //theTextArea.setCaretPosition(caret-1); // Used for placing the cursor at the end.
              out.write(ch);
              nbrOfKeyTyped--;
              consume = false;
            }
             ...
             if (nbrOfKeyTyped > 0) {
                 String text = theTextArea.getText();
                 //int caret = theTextArea.getCaretPosition();
                 theTextArea.setText(text.substring(0, text.length() - nbrOfKeyTyped));
                 theTextArea.append(""); // Used for placing the cursor at the end.
                 //theTextArea.setCaretPosition(caret +1000);//-nbrOfKeyTyped); // Used for placing the cursor at the end.
                 out.write(21);
              }
    ...
    static public void main(String [] args) throws Exception {
        Console c = new Console(new FESI.gui.InterpreterCommands()
